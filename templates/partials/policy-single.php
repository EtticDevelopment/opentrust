...
<main id="ettic-otc-main" class="ettic-otc-policy-single">
    <div class="ettic-otc-container ettic-otc-container--narrow">

        <a href="<?php echo esc_url($ot_base_url); ?>" class="ettic-otc-policy-single__back">
            &larr; <?php esc_html_e('Back to Trust Center', 'open-trust-center-by-ettic'); ?>
        </a>

        <?php if ($ot_is_old_version): ?>
        <div class="ettic-otc-version-banner">
            ...
            printf(
                ...
                esc_html__('You are viewing version %1$s. %2$s', 'open-trust-center-by-ettic'),
                esc_html((string) $ot_version),
                '<a href="' . esc_url($ot_current_url) . '">' . esc_html__('View current version', 'open-trust-center-by-ettic') . '</a>'
            );
            ?>
        </div>
        <?php elseif ($ot_is_pending): ?>
        <div class="ettic-otc-version-banner ettic-otc-version-banner--pending">
            ...
            printf(
                ...
                esc_html__('This version takes effect on %1$s. %2$s', 'open-trust-center-by-ettic'),
                esc_html($ot_effective_date),
                $ot_version > 1
                    ? '<a href="' . esc_url($ot_current_url . 'version/' . ($ot_version - 1) . '/') . '">' . esc_html__('View current version', 'open-trust-center-by-ettic') . '</a>'
                    : ''
            );
            ?>
        </div>
        ...
        <div class="ettic-otc-policy-single__header">
            <div class="ettic-otc-policy-single__eyebrow">
                <?php if ($ot_ref_id !== ''): ?>
                <span class="ettic-otc-policy-single__ref"><?php echo esc_html($ot_ref_id); ?></span>
                <?php endif; ?>
                <span class="ettic-otc-pill ettic-otc-pill--category"><?php echo esc_html($ot_category_label); ?></span>
                <span class="ettic-otc-policy-single__version">
                    <?php
                    /* translators: %s: policy version number */
                    printf(esc_html__('v%s', 'open-trust-center-by-ettic'), esc_html((string) $ot_version)); ?>
                </span>
            </div>

            <h1 class="ettic-otc-policy-single__title"><?php echo esc_html($ot_policy->post_title); ?></h1>

            <div class="ettic-otc-policy-single__meta">
                <?php if ($ot_effective_date): ?>
                <span class="ettic-otc-policy-single__meta-item">
                    <?php
                    /* translators: %s: policy effective date */
                    printf(esc_html__('Effective %s', 'open-trust-center-by-ettic'), esc_html($ot_effective_date)); ?>
                </span>
                <?php endif; ?>
                <span class="ettic-otc-policy-single__meta-item">
                    <?php
                    /* translators: %s: policy last updated date */
                    printf(esc_html__('Updated %s', 'open-trust-center-by-ettic'), esc_html($ot_modified_date)); ?>
                </span>
            </div>

            <?php if (!empty($ot_citations)): ?>
            <ul class="ettic-otc-policy-single__citations" role="list" aria-label="<?php esc_attr_e('Framework citations', 'open-trust-center-by-ettic'); ?>">
                <?php foreach ($ot_citations as $ot_citation): ?>
                <li class="ettic-otc-policy-single__citation"><?php echo esc_html($ot_citation); ?></li>
                <?php endforeach; ?>
            </ul>
            <?php endif; ?>

            <div class="ettic-otc-policy-single__actions">
                <?php if ($ot_attachment): ?>
                <a href="<?php echo esc_url($ot_attachment['url']); ?>" class="ettic-otc-btn ettic-otc-btn--primary" download>
                    ...
                    if (!empty($ot_attachment['size_human'])) {
                        ...
                        printf(esc_html__('Download PDF (%s)', 'open-trust-center-by-ettic'), esc_html($ot_attachment['size_human']));
                    } else {
                        esc_html_e('Download PDF', 'open-trust-center-by-ettic');
                    }
                    ?>
                </a>
                <?php endif; ?>
                <button type="button" class="ettic-otc-btn ettic-otc-btn--outline" onclick="window.print()">
                    ...
                    <?php esc_html_e('Print', 'open-trust-center-by-ettic'); ?>
                </button>
            </div>
        </div>

        <?php if (count($ot_versions) > 1): ?>
        <div class="ettic-otc-version-history-public">
            <button class="ettic-otc-version-history-toggle" data-ettic-otc-version-toggle aria-expanded="false">
                ...
                printf(esc_html__('Version history (%d)', 'open-trust-center-by-ettic'), count($ot_versions)); ?>
                <svg class="ettic-otc-version-history-chevron" width="14" height="14" viewBox="0 0 20 20" fill="currentColor" aria-hidden="true"><path fill-rule="evenodd" d="M5.293 7.293a1 1 0 011.414 0L10 10.586l3.293-3.293a1 1 0 111.414 1.414l-4 4a1 1 0 01-1.414 0l-4-4a1 1 0 010-1.414z" clip-rule="evenodd"/></svg>
            </button>
            <ul class="ettic-otc-version-list" hidden>
                <?php foreach ($ot_versions as $ot_v):
                    $ot_is_active = $ot_v['version'] === $ot_version;
                    $ot_classes   = 'ettic-otc-version-list__link' . ($ot_is_active ? ' ettic-otc-version-list__link--current' : '');
                ?>
                <li class="ettic-otc-version-list__item<?php echo esc_attr($ot_is_active ? ' ettic-otc-version-list__item--active' : ''); ?>">
                    <?php if ($ot_is_active): ?>
                    <span class="<?php echo esc_attr($ot_classes); ?>">
                    <?php else: ?>
                    <a href="<?php echo esc_url($ot_v['url']); ?>" class="<?php echo esc_attr($ot_classes); ?>">
                    <?php endif; ?>
                        <span class="ettic-otc-version-list__number"><?php
                        /* translators: %d: version number */
                        printf(esc_html__('v%d', 'open-trust-center-by-ettic'), (int) $ot_v['version']); ?></span>
                        <span class="ettic-otc-version-list__sep">&middot;</span>
                        <span class="ettic-otc-version-list__date"><?php echo esc_html($ot_v['date']); ?></span>
                        <?php if (!empty($ot_v['summary'])): ?>
                            <span class="ettic-otc-version-list__sep">&middot;</span>
                            <span class="ettic-otc-version-list__summary"><?php echo esc_html($ot_v['summary']); ?></span>
                        <?php endif; ?>
                        <?php if ($ot_v['current']): ?>
                            <span class="ettic-otc-version-list__badge"><?php esc_html_e('Current', 'open-trust-center-by-ettic'); ?></span>
                        <?php endif; ?>
                    <?php if ($ot_is_active): ?>
                    </span>
                    <?php else: ?>
        ...
        <article class="ettic-otc-policy-content">
